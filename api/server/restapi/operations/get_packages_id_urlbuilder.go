...
func (o *GetPackagesIDURL) Build() (*url.URL, error) {
	...
	var _path = "/packages/{id}"

	id := o.ID
	if id != "" {
		_path = strings.Replace(_path, "{id}", id, -1)
	} else {
		return nil, errors.New("id is required on GetPackagesIDURL")
	}
...
func (o *GetPackagesIDURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on GetPackagesIDURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on GetPackagesIDURL")
	}
