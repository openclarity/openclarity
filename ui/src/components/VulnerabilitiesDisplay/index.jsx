        ...
        ({ totalKey, color }) => (
          <div key={totalKey} className="vulnerabilities-tooltip-counters-item">
            <Icon
              ...
              size={18}
            />
            <span>{formatNumber(counters[totalKey] || 0)}</span>
          </div>
        ),
      )}
    </div>
...
const MinimizedVulnerabilitiesDisplay = ({
  ...
  return (
    <div className="vulnerabilities-minimized-display-wrapper">
      <TooltipWrapper
        tooltipId={`vulnerability-minimized-display-${id}`}
        tooltipText={
          <TooltipContentDisplay total={total} counters={counters} />
        }
      >
        <div
          className="vulnerabilities-minimized-display-summary-item"
          style={{
...
const CounterItemDisplay = ({ count, title, color }) => (
  <div className="vulnerabilities-display-counter-item">
    <div className="vulnerabilities-counter-item-count" style={{ color }}>
      {count || 0}
    </div>
      ...
      <div className="vulnerabilities-display-counters">
        {Object.values(VULNERABILITY_SEVERITY_ITEMS).map(
          ({ totalKey, title, color }) => (
            <CounterItemDisplay
              key={totalKey}
              count={formatNumber(counters[totalKey])}
              title={title}
              color={color}
            />
          ),
