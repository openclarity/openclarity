...
const calcRange = (unit, value) => ({
  endTime: moment().toISOString(),
  startTime: moment().subtract(unit, value).toISOString(),
});
...
const WidgetChart = ({ data, selectedFilters }) => {
  const formattedData = data?.reduce((acc, curr) => {
    ...
    trends.forEach(({ time, count }) => {
      const accTimeIndex = acc.findIndex(
        ({ time: accTime }) => time === accTime,
      );
      const formattedTime = formatDate(time);

      acc =
        accTimeIndex < 0
          ? [...acc, { time, formattedTime, [findingType]: count }]
          : [
  ...
  return (
    <div
      className="findings-trends-widget-chart"
...
const FindingsTrendsWidget = ({ className }) => {
  ...
  return (
    <WidgetWrapper
      className={classnames("findings-trends-widget", className)}
      title="Findings trends"
    >
      <div className="findings-trends-widget-header">
        ...
        <DropdownSelect
          ...
          value={selectedRange}
          items={Object.values(TIME_RANGES).map(({ value, label }) => ({
            value,
            label,
          }))}
