      ...
      <thead>
        <tr>
          {getHeaderItems(selectedId).map((item, index, items) => (
            <th
              key={index}
              style={items.length - 1 === index ? { textAlign: "right" } : {}}
            >
              {item}
            </th>
          ))}
      ...
      <tbody>
        {displayData.length > 0 ? (
          displayData.map((item, index) => {
            return (
              <tr key={index}>
                {getBodyItems(selectedId).map(
                  ({ dataKey, customDisplay: CustomDisplay }, index, items) => (
                    <td
                      key={index}
                      style={
                        items.length - 1 === index ? { textAlign: "right" } : {}
                      }
                    >
                      {!!CustomDisplay ? (
                        <CustomDisplay {...item} />
...
const Tab = ({ widgetName, title, icon, isActive }) => (
  <IconWithTooltip
    ...
    name={icon}
    style={{
      color: isActive ? COLORS["color-grey-black"] : COLORS["color-grey"],
    }}
    size={20}
  />
);

const FindingsTabsWidget = ({
  widgetName,
  findingsItems,
  className,
  title,
  ...
  return (
    <WidgetWrapper
      title={title}
      className={classnames("findings-tabs-widget", className)}
      titleMargin={10}
    >
      <Tabs
