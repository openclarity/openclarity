...
import VulnerabilitiesDisplay, {
  getTotlalVulnerabilitiesFromCounters,
      ...
      filters: {
        ...
        customDisplay: () => (
          <FindingsSystemFilterLinks
            findingsSummary={findingsSummary}
            totalVulnerabilitiesCount={getTotlalVulnerabilitiesFromCounters(
            ...
            items={[
              {
                path: pathname,
                component: () => (
                  <VulnerabilitiesDisplay counters={totalVulnerabilities} />
                ),
                callback: onFindingsClick,
              },
              ...Object.keys(FINDINGS_MAPPING).map((findingType) => {
                const { totalKey, title, icon, color, value } =
                  FINDINGS_MAPPING[findingType];

                return {
                  path: pathname,
                  component: () => (
                    <FindingsCounterDisplay
                      key={totalKey}
                      icon={icon}
                      color={color}
                      count={findingsSummary[totalKey] || 0}
                      title={title}
                    />
                  ),
                  callback: onFindingsClick,
