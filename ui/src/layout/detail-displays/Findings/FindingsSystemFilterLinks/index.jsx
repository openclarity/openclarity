...
const FindingFilterLink = ({ icon, title, appRoute }) => {
  ...
  return (
    <div className="finging-filter-link">
      ...
      <Button tertiary onClick={() => navigate(appRoute)}>
        {title}
      </Button>
    </div>
  );
...
const FindingsSystemFilterLinks = ({
  ...
  <div className="findings-system-filters-links">
    ...
    {[VULNERABILITY_FINDINGS_ITEM, ...Object.values(FINDINGS_MAPPING)].map(
      ({ value, totalKey, title, icon }) => {
        const LinkTitle =
          VULNERABILITY_FINDINGS_ITEM.value === value
            ? `${formatNumber(totalVulnerabilitiesCount)} ${title}`
        ...
        return (
          <FindingFilterLink
            ...
            icon={icon}
            title={LinkTitle}
            appRoute={getFindingsAbsolutePathByFindingType(value)}
          />
        );
      },
    )}
