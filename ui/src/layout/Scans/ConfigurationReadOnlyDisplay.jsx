...
const FlagPropDisplay = ({ checked, label }) => (
  ...
  >{`${label} ${checked ? "enabled" : "disabled"}`}</div>
);
...
const ConfigurationReadOnlyDisplay = ({ configData }) => {
  ...
  const { scannerInstanceCreationConfig, scanFamiliesConfig } =
    assetScanTemplate;
    ...
    <>
      ...
      <TitleValueDisplayRow>
        <>
          ...
          <div>
            {!!cronLine
              ? cronExpressionToHuman(cronLine)
      ...
      <TitleValueDisplayRow>
        <TitleValueDisplay title="Scan types">
          <ValuesListDisplay
            values={getEnabledScanTypesList(scanFamiliesConfig)}
          />
        </TitleValueDisplay>
        <TitleValueDisplay title="Advanced settings">
          <FlagPropDisplay
            label="Spot instances required"
