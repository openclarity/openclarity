...
const SCAN_TYPES_FILTER_ITEMS = [
  ...
  value: `scanTemplate.assetScanTemplate.scanFamiliesConfig.${type}.enabled`,
  label: toCapitalized(type),
}));
...
const formatScanTypesToOdata = (valuesList, operator) =>
  valuesList
    .map(
      (value) =>
        `(${value} eq ${operator === OPERATORS.contains.value ? "true" : "false"})`,
    )
    .join(` or `);
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        Cell: ({ row }) => {
          const { scanFamiliesConfig } =
            row.original.scanTemplate.assetScanTemplate;
      ...
      <TablePage
        ...
        filtersConfig={[
          ...
          },
          {
            value: "scanTypes",
            label: "Scan types",
            customOdataFormat: formatScanTypesToOdata,
            operators: [
