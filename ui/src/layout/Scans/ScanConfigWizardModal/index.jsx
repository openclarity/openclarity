...
const ScanConfigWizardModal = ({ initialData, onClose, onSubmitSuccess }) => {
  ...
  const { scanFamiliesConfig, scannerInstanceCreationConfig } =
    assetScanTemplate || {};
  ...
  const initialValues = {
    ...
    scheduled: {
      scheduledSelect: !!cronLine
        ? SCHEDULE_TYPES_ITEMS.REPETITIVE.value
        : SCHEDULE_TYPES_ITEMS.NOW.value,
      laterDate: "",
      laterTime: "",
  ...
  Object.keys(scanFamiliesConfig || {}).forEach((type) => {
    ...
    initialValues.scanTemplate.assetScanTemplate.scanFamiliesConfig[
      type
    ].enabled = enabled;
  });
