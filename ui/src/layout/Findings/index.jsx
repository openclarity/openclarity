...
export const FINDINGS_PATHS = Object.keys(FINDINGS_TAB_ITEMS).reduce(
  (acc, curr) => ({ ...acc, [curr]: FINDINGS_TAB_ITEMS[curr].path }),
  {},
);

export const getFindingsAbsolutePathByFindingType = (type) => {
  const relativePath = Object.values(FINDINGS_TAB_ITEMS).find(
    ({ findingsType }) => type === findingsType,
  )?.path;
