...
const ConnectedNotification = () => {
  ...
  return (
    <Notification
      message={message}
      type={type}
      onClose={() => removeNotification(dispatch)}
    />
  );
};
...
const Layout = () => {
  ...
  const pageTitle = ROUTES_CONFIG.find(
    ({ path, isIndex }) => (isIndex && !mainPath) || path === `/${mainPath}`,
  )?.title;
      ...
      <FiltersProvider>
        <div className="topbar-container">
          ...
          <div className="topbar-menu-items">
            <a href="/apidocs" target="_blank" className="topbar-api-link">
              API Docs
            </a>
          </div>
        </div>
        <div className="sidebar-container">
          {ROUTES_CONFIG.map(
            ({ path, icon, title, resetFilters, resetFilterAll }) => (
              <TooltipWrapper
                key={path}
                tooltipId={`sidebar-item-tooltip-${path}`}
                tooltipText={title}
              >
                <NavLinkItem
                  pathname={path}
                  icon={icon}
                  resetFilterNames={resetFilters}
