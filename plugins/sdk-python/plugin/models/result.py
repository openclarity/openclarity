from datetime import date, datetime  # noqa: F401

from typing import List, Dict  # noqa: F401

from plugin.models.base_model import Model
from plugin.models.open_clarity_data import OpenClarityData
from plugin import util

from plugin.models.open_clarity_data import OpenClarityData  # noqa: E501

class Result(Model):
    """NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).

    Do not edit the class manually.
    """

    def __init__(self, annotations=None, openclarity=None, raw_json=None, raw_sarif=None):  # noqa: E501
        """Result - a model defined in OpenAPI

        :param annotations: The annotations of this Result.  # noqa: E501
        :type annotations: Dict[str, str]
        :param openclarity: The openclarity of this Result.  # noqa: E501
        :type openclarity: OpenClarityData
        :param raw_json: The raw_json of this Result.  # noqa: E501
        :type raw_json: object
        :param raw_sarif: The raw_sarif of this Result.  # noqa: E501
        :type raw_sarif: object
        """
        self.openapi_types = {
            'annotations': Dict[str, str],
            'openclarity': OpenClarityData,
            'raw_json': object,
            'raw_sarif': object
        }

        self.attribute_map = {
            'annotations': 'annotations',
            'openclarity': 'openclarity',
            'raw_json': 'rawJSON',
            'raw_sarif': 'rawSarif'
        }

        self._annotations = annotations
        self._openclarity = openclarity
        self._raw_json = raw_json
        self._raw_sarif = raw_sarif

    @classmethod
    def from_dict(cls, dikt) -> 'Result':
        """Returns the dict as a model

        :param dikt: A dict.
        :type: dict
        :return: The Result of this Result.  # noqa: E501
        :rtype: Result
        """
        return util.deserialize_model(dikt, cls)

    @property
    def annotations(self) -> Dict[str, str]:
        """Gets the annotations of this Result.

        Generic map of string keys and string values to attach arbitrary non-identifying metadata to objects.  # noqa: E501

        :return: The annotations of this Result.
        :rtype: Dict[str, str]
        """
        return self._annotations

    @annotations.setter
    def annotations(self, annotations: Dict[str, str]):
        """Sets the annotations of this Result.

        Generic map of string keys and string values to attach arbitrary non-identifying metadata to objects.  # noqa: E501

        :param annotations: The annotations of this Result.
        :type annotations: Dict[str, str]
        """

        self._annotations = annotations

    @property
    def openclarity(self) -> OpenClarityData:
        """Gets the openclarity of this Result.


        :return: The openclarity of this Result.
        :rtype: OpenClarityData
        """
        return self._openclarity

    @openclarity.setter
    def openclarity(self, openclarity: OpenClarityData):
        """Sets the openclarity of this Result.


        :param openclarity: The openclarity of this Result.
        :type openclarity: OpenClarityData
        """
        if openclarity is None:
            raise ValueError("Invalid value for `openclarity`, must not be `None`")  # noqa: E501

        self._openclarity = openclarity

    @property
    def raw_json(self) -> object:
        """Gets the raw_json of this Result.

        Defines scan result data that is not consumed by OpenClarity API.  # noqa: E501

        :return: The raw_json of this Result.
        :rtype: object
        """
        return self._raw_json

    @raw_json.setter
    def raw_json(self, raw_json: object):
        """Sets the raw_json of this Result.

        Defines scan result data that is not consumed by OpenClarity API.  # noqa: E501

        :param raw_json: The raw_json of this Result.
        :type raw_json: object
        """
        if raw_json is None:
            raise ValueError("Invalid value for `raw_json`, must not be `None`")  # noqa: E501

        self._raw_json = raw_json

    @property
    def raw_sarif(self) -> object:
        """Gets the raw_sarif of this Result.

        Defines scan result data in that is not consumed by the OpenClarity API.  # noqa: E501

        :return: The raw_sarif of this Result.
        :rtype: object
        """
        return self._raw_sarif

    @raw_sarif.setter
    def raw_sarif(self, raw_sarif: object):
        """Sets the raw_sarif of this Result.

        Defines scan result data in that is not consumed by the OpenClarity API.  # noqa: E501

        :param raw_sarif: The raw_sarif of this Result.
        :type raw_sarif: object
        """

        self._raw_sarif = raw_sarif
