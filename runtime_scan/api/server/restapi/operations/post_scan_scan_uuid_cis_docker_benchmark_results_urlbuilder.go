...
func (o *PostScanScanUUIDCisDockerBenchmarkResultsURL) Build() (*url.URL, error) {
	...
	var _path = "/scan/{scan-uuid}/cisDockerBenchmark/results"

	scanUUID := o.ScanUUID.String()
	if scanUUID != "" {
		_path = strings.Replace(_path, "{scan-uuid}", scanUUID, -1)
	} else {
		return nil, errors.New("scanUuid is required on PostScanScanUUIDCisDockerBenchmarkResultsURL")
	}
...
func (o *PostScanScanUUIDCisDockerBenchmarkResultsURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on PostScanScanUUIDCisDockerBenchmarkResultsURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on PostScanScanUUIDCisDockerBenchmarkResultsURL")
	}
